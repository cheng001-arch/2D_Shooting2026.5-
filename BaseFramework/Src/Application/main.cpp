...
void Application::DrawSprite()
{
	...
	KdShaderManager::Instance().m_spriteShader.Begin();
	{
		if (m_playerTex.GetSRView())
		{
			...
			const int drawWidth = static_cast<int>(frameWidth * m_playerDrawScale);
			const int drawHeight = static_cast<int>(frameHeight * m_playerDrawScale);

			KdShaderManager::Instance().m_spriteShader.DrawTex(&m_playerTex,
				static_cast<int>(m_playerPos.x), static_cast<int>(m_playerPos.y),
				drawWidth, drawHeight, &srcRect);
		}
	}
